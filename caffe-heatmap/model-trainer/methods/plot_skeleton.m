%PLOT_SKELETON - plots skeleton of signer on figure
...
function handle = plot_skeleton(j,conf,thresh,opts,handle)
    ...
    if isempty(opts)
        opts.clr = jet(7); %lines(11) %sets colour of joints
        opts.clr(8,:) = [1 0 0];
        opts.clr(9,:) = [0 1 0];
        opts.linewidth = 4;
        opts.jointsize = 8;
        ...
        for c = 1:11
            ...
            if conf(c) > thresh
                if false%c == 1
                    ...
                    'markerfacecolor',[0 0 0], 'markersize',50,'linewidth',opts.jointlinewidth(c),'color',opts.jointlinecolor(c,:));
                else
                handle.joints(c) =  plot(handle.axis,j(1,c),j(2,c),'bo', ...
