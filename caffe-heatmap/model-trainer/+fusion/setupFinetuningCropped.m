function [filename,folder] =  setupFinetuningCropped(cnnopts,exp_name,videoname,videoFilename,trainFrameids,trainLocs,testFrameids,testLocs,imscale,bbWidth,bbHeight,dateTime)
...
%CROPPED VERSION - trains by cropping out a 256 by 256 bounding box around the person
    ...
    runstr = sprintf('%sbuild/tools/caffe train -gpu 0 -solver %s  2>&1 | tee -a %strain_log.txt ',cnnopts.finetune.cafferoot,filename.solver,folder.model);
    filename.bash= sprintf('%strain_%s.sh',folder.model,dateTime);
