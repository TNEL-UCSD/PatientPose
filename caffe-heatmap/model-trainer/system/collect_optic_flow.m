...
function collect_optic_flow(videoname,exp_name)
    ...
    if flow_file.flow_count < numFrames
        for i = startid:(numFrames-1)
            ...
            sys_str = sprintf('%s %s %s %s -middlebury',opts.deepflowstatic,infilename1, infilename2, outfilename);
            sysmsg = system(sys_str);
            
%             im1 = single(imread(infilename1));
%             im2 = single(imread(infilename2));     
%             matches = deepmatching(im1,im2);
%             flow = deepflow2(im1, im2, matches, '-middlebury');
