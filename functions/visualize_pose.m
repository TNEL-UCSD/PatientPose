function [] = visualize_pose(tnelOpt, im, joints, heatmapVis)

% Visualize the output of Caffe-Heatmap
if tnelOpt.visualizeoutput1
    ...
    img = imgaussfilt(imread(im.namesSaveNatSort{1}),tnelOpt.blurimagesigma256);
    ...
    for i = 1:length(im.namesSaveNatSort)
        ...
        if tnelOpt.visualizeoutput1Skel
            disp(['Saving skeleton overlay image ' num2str(i) ' of ' num2str(length(im.namesSaveNatSort))]);
            
            % Process image
            if tnelOpt.blurimage
                img = uint8(imgaussfilt(imread(im.namesSaveNatSort{i}), tnelOpt.blurimagesigma256));
            else
        ...
        if tnelOpt.visualizeoutput1Heatmap
